...
public class CharacterMovement : BaseMonoBehaviour, IMovementMotor, IJumpMovement
{
    ...
    public Vector2 Velocity =>
        this._rigidbody2D == null
            ? Vector2.zero
            : this._rigidbody2D.linearVelocity;
    ...
    public virtual void Move(Vector2 direction)
    {
        ...
        float horizontalVelocity = horizontalDirection * this._moveSpeed;

        this._rigidbody2D.linearVelocity = new Vector2(
            horizontalVelocity,
            this._rigidbody2D.linearVelocity.y
        );

        this.FaceMoveDirection(horizontalDirection);
    }
    ...
    protected virtual void FaceMoveDirection(float horizontalDirection)
    {
        ...
        Vector3 localScale = this._player.localScale;

        localScale.x = Mathf.Abs(localScale.x) *
                       (this._isFacingRight ? 1f : -1f);

        this._player.localScale = localScale;
    }
    ...
    protected virtual void LoadPlayer()
    {
        ...
        PlayerController playerController =
            transform.GetComponentInParent<PlayerController>();

        if (playerController == null) return;

        this._player = playerController.transform;
    }
    ...
    protected virtual void LoadColider()
    {
        ...
        this._collider2D =
            transform.GetComponentInParent<Collider2D>();
    }
